...
class QwenVLModel(BaseCaptionModel):
    ...
    def load(self) -> None:
        ...
        load_start = log_start("Qwen", f"Loading {model_name} ({precision_label})")
        ...
        log_end("Qwen", f"{model_name} loaded", load_start)
    ...
    def generate_text_only(
        ...
        with isolated_execution():
            ...
            # Build messages in Qwen format (text-only, no image)
            messages = [
                {
                    "role": "user",
                    "content": [
