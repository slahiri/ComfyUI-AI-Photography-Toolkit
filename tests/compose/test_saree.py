"""Quick test of Phase 2 with saree image metadata."""
...
def main():
    print("=" * 70)
    print("Phase 2 Test: Saree Image")
    print("=" * 70)

    # Load metadata
    sample_path = Path(__file__).parent / "sample_metadata_saree.json"
    ...
    print("\n--- Key Tokens for Saree Image ---")
    keywords = ["saree", "sari", "indian", "traditional", "black hair", "red", "bracelet", "standing", "posing"]
    found = []
    ...
    for kw, text, conf, src in sorted(set(found), key=lambda x: x[2], reverse=True):
        print(f"  [{conf:.2f}] '{text}' ({src})")
